...
::: {.callout-caution title="Atención"}
No siempre se tiene información sobre qué valor es el mejor para el parámetro $k$ en el modelo de las $k$-medias. De hecho, en ocasiones el interés de los métodos de agrupamiento es precisamente averiguar dicho valor.
:::
...
Como hemos comentado anteriormente, en numerosas ocasiones el número de clústeres de un problema no supervisado lo establece el dominio de aplicación, o más concretamente nuestra necesidad de comunicar los resultados y que estos tengan sentido y sean lo más explicables posibles.
...
A continuación mostramos los tres métodos más populares para determinar el número de clústeres: método del *codo* , *silhoutte* y método estadístico de la *brecha* (Gap).
...
En este caso parece que $4$ es una buena elección para el número de clusters. De modo que:
...
4.  Selección del número de clusters: El valor de $k$ que maximiza el estadístico anterior se considera el número óptimo de clusters. En otras palabras, se elige el valor de $k$ donde la brecha entre los datos reales y los datos de referencia es más grande.
...
Tal y como hacíamos en el clustering no jerárquico, podemos aplicar métodos para determinar el número de clusters. Por ejemplo, el método del codo:

```{r}
fviz_nbclust(df, function(x, k){

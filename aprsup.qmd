...
-   Se optimiza el parámetro $\alpha_m$, que controla la tasa de aprendizaje $\eta$ y la magnitud de la corrección del modelo en cada paso, mediante el descenso de gradiente: $$\alpha_m = arg min_{\alpha} \sum_{i=1}^{n} L(y_i,F_{m-1}(\mathbf{x}_i+\alpha f_{m}(\mathbf{x}_i)))$$

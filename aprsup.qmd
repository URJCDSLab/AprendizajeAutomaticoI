...
df.train$deposit <- as.numeric(as.factor(df.train$deposit)) - 1
df.test$deposit  <- as.numeric(as.factor(df.test$deposit)) - 1

# One-hot encoding 
train_matrix <- model.matrix(deposit ~ . -1, data = df.train)
test_matrix  <- model.matrix(deposit ~ . -1, data = df.test)

#  DMatrix 
dtrain <- xgb.DMatrix(data = train_matrix, label = df.train$deposit)
dtest  <- xgb.DMatrix(data = test_matrix,  label = df.test$deposit)
...
xgb.plot.multi.trees(model = modelo_01)
```
...
importance_matrix <- xgb.importance(model = modelo_01)
...
pred_prob <- predict(modelo_01, dtest)

pred_class <- pred_prob > 0.5

# Tabla + matriz de confusión
cbind(pred_class, df.test$deposit) %>% 
  data.frame() %>% 
  table() %>% 
...
# Modelo Naive Bayes
model <- naive_bayes(as.factor(deposit) ~ ., data = df.train, usekernel = TRUE)

# Probabilidades
probabilities <- predict(model, df.test %>% dplyr::select(-deposit), type = "prob")[,2]

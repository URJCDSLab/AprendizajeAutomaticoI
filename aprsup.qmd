...
$$ P(Y=c|\mathbf{X}=\mathbf{x})=\frac{P(Y=c|\mathbf{x})P(Y=c)}{P(\mathbf{x})}$$
...
LDA sigue la misma filosofía que las PCA pero se centra en maximizar la separabilidad entre las categorías.

Una forma intuitiva de ver el LDA es como una proyección geométrica de los datos en una recta, donde la asignación de clase se realiza según la proximidad a las medias proyectadas de cada clase. LDA busca encontrar un vector de proyección $\mathbf{w}$ tal que los datos de distintas clases queden lo más separados posibles de sí. Matemáticamente, dicha proyección es $\mathbf{z}=\mathbf{w}^t\mathbf{x}$ (¿Te recuerda a las PCA?) donde $\mathbf{z}$ representa la coordenada del punto $\mathbf{x}$ en la nueva dimensión reducida.

En el caso de LDA, para encontrar ese nuevo eje, lo que se busca es que las medias de las clases estén lo más separadas posibles y que su dispersión sea lo más pequeña posible. Con este en mente, obtengamos dicho $\mathbf{w}$ para el problema binario. Las medias de cada clase son $$\mu_{0} = \frac{1}{n_{0}} \sum_{i:Y_{i}=0} \mathbf{x}_i$$ y $$\mu_{1} = \frac{1}{n_{1}} \sum_{i:Y_{i}=1} \mathbf{x}_i$$.

Y sea $m_c = \mathbf{w}^{t}\mu_{c}$ la proyección de cada media en $\mathbf{w}$. Del mismo modo, $\mathbf{z}_i=\mathbf{w}^t\mathbf{x}_i$ es la proyección de los datos. La varianza de los puntos proyectos es $$S_{c}^{2}=\sum_{i:Y_i=c}(z_i-m_c)^{2}.$$ Como lo que queremos es maximizar la distancia entre las medias $m_0$ y $m_1$ y que la proyección de cada clase tenga poca dispersión, la función a maximizar es $$J(\mathbf{w})=\frac{(m_0-m_1)^{2}}{S_{0}^{2} + S_{1}^{2}}$$ que se puede reescribir como $$J(\mathbf{w})=\frac{\mathbf{w}^{t}S_{B}\mathbf{w}}{\mathbf{w}^{t}S_{W}\mathbf{w} },$$ siendo $S_{B}=(\mu_1 - \mu_0)(\mu_1 - \mu_0)^{t}$ la varianza entre clases (between) y $S_{W}=\sum_{i:Y_i=0}(\mathbf{x}_i - \mu_0)(\mathbf{x}_i - \mu_0)^{t} + \sum_{i:Y_i=1}(\mathbf{x}_i - \mu_1)(\mathbf{x}_i - \mu_1)^{t}$ la varianza dentro de cada clase (within).

Derivando $J(\mathbf{w})$ con respecto $\mathbf{w}$ y realizando la correspondiente descomposición en valores singulares se llega a $\mathbf{w}=S_{W}^{-1}(\mu_1 - \mu_0)$, que es lo que obtuvimos antes.
:::
